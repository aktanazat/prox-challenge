...
- **Field knowledge layer**: a second corpus (`knowledge/community/`) curated from primary-source research on r/Welding, WeldingWeb, Garage Journal, and 1,400+ owner reviews — known firmware preset errors (the synergic table is provably wrong in documented ways), failure modes with warranty math, generator compatibility, feed-path triage, TIG reality checks. Every field fact carries its source URL and renders as a distinct ▴field chip, never disguised as manual content. When machine and field disagree, the agent shows both.
- **The Door Chart**: welders tape settings to the machine, not PDFs. Ask for one and the agent generates a printable, laminate-ready settings card for your wire/gas/voltage — machine preset, field-corrected start value, and a blank column for your own dialed-in numbers. Print button on the card.
- **Diagnoses like a pro**: subsystem isolation first ("does it arc in Stick?"), ground and connectors before boards, the six questions before any settings number, generator flow on trigger, cheapest-first feed-path triage — the exact question order extracted from real forum diagnostic threads. Weld photos get anti-sycophancy rules: sufficiency check first, every defect localized, fitness-for-purpose verdict first, no flattery.
